<?php
	...
	namespace DojahCore\Dojah\Http;

	use \DojahCore\Dojah\Contracts\RouteInterface;
	...
	class HttpClient {
		...
		public function checkResp($response)
		{
			...
			if ($status !== 200) {
				throw new \Exception("Something went wrong.: " . $response['error'] . " : " . $response["status_code"]);
			}
